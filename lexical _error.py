...
<<<<<<< HEAD
def path_vowels(url, total_num_chars):             #checked
    parse=urlparse(url)
    path=parse[2]
    vowA=('Aa')
    countap=0
...
def url_vowels(url, total_num_chars): #checked
    vowA=('Aa')
    countA=0
    vowE=('eE')
    countE=0
...
def url_characterfreq(url):                #checked
    character_count=0
    url_characters=('!@#$%^&*()-_=+{}[]|\/:;><,')
    for character in url:
        if character in url_characters:
            character_count = character_count + 1
    print(character_count)
    return character_count

def path_characterfreq(url):            #checked
    parse=urlparse(url)
    path_characters=('!@#$%^&*()-_=+{}[]|\/:;><,?')
    path=parse[2]
    path_charactercounter=0
    for character in path:
        if character in path_characters:
...
def get_domain_tokens(url):            #checked
=======
...
def get_path_tokens(url):           #checked
    temp = url.find('/')
    path = url[temp+1:]
...
<<<<<<< HEAD
def token_characteristics(tokens):        #checked
    print(tokens)
=======
